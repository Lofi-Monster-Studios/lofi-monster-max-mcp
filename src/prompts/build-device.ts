...
export function register(server: McpServer) {
  server.prompt(
    "build-device",
    "Guided workflow for building a complete Max for Live device from scratch",
    { deviceName: z.string().describe("Name of the device to build (e.g. 'Flutter')") },
    async ({ deviceName }) => ({
      messages: [
        {
          role: "user" as const,
          content: {
            type: "text" as const,
            text: `Build a Max for Live device called "${deviceName}" using the LOFI MONSTER MCP tools.
