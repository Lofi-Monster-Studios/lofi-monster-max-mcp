...
export function register(server: McpServer) {
  server.prompt(
    "constraints",
    "Inject all 12 Max/MSP constraints into context to prevent hallucinated API calls",
    async () => ({
      messages: [
        {
          role: "user" as const,
          content: {
            type: "text" as const,
            text: `Before generating any Max/MSP code or using LOFI MONSTER tools, you MUST respect these 12 constraints. Violating any of them will produce silent failures or crashes in Max.
